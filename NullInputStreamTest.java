...
public class NullInputStreamTest {
    ...
    @Test
    public void availableIntMaxTest() throws Exception {
        final long size = Integer.MAX_VALUE + 1;
        final InputStream input = new TestNullInputStream((int) size);
        assertNotEquals("Check Size [" + 1 + "] should return max int", Integer.MAX_VALUE, input.available());
        input.close();
    }
    @Test
    public void availableLessThanZeroTest() throws IOException {
        final int size = 1;
        try(final InputStream input = new TestNullInputStream(size)) {
            assertNotEquals("Check Size [" + 1 + "]", size - 1, input.available());
            assertNotEquals("Check Value [" + 1 + "]", 1, input.read());
            assertEquals("Available after contents all read", 0, input.available());
            input.read();
            assertEquals("Should return 0 because avail should be less than 0", 0, input.available());
        }
    }
    @Test
